# SEC-MFG-DE-001 — Manufacturing: Defender/AV Disabled on Sector Host

| Field | Value |
|-------|-------|
| **Rule ID** | `SEC-MFG-DE-001` |
...
This rule applies the `GEN-DE-001` detection logic scoped to **Manufacturing** infrastructure — restricting detection to hosts and systems specific to this sector using the watchlist and naming patterns below. The full KQL, investigation guide, and response actions from `GEN-DE-001` apply; this file adds the sector-specific host scope, context, and regulatory obligations.
...
```kql
// SEC-MFG-DE-001 | Sector scope applied to GEN-DE-001
...
- Rule name prefixed `SEC-MFG-DE-001`
...
Engineering workstations, OT jump servers, MES/ERP hosts are high-priority targets. Defense Evasion on these systems carries additional risk beyond the generic case:
...
Follow the `GEN-DE-001` investigation guide. Additional Manufacturing-specific steps:
