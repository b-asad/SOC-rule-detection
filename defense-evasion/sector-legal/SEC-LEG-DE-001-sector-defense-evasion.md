# SEC-LEG-DE-001 — Legal: Defender/AV Disabled on Sector Host

| Field | Value |
|-------|-------|
| **Rule ID** | `SEC-LEG-DE-001` |
...
This rule applies the `GEN-DE-001` detection logic scoped to **Legal** infrastructure — restricting detection to hosts and systems specific to this sector using the watchlist and naming patterns below. The full KQL, investigation guide, and response actions from `GEN-DE-001` apply; this file adds the sector-specific host scope, context, and regulatory obligations.
...
```kql
// SEC-LEG-DE-001 | Sector scope applied to GEN-DE-001
...
- Rule name prefixed `SEC-LEG-DE-001`
...
Fee-earner workstations, DMS hosts, matter management servers are high-priority targets. Defense Evasion on these systems carries additional risk beyond the generic case:
...
Follow the `GEN-DE-001` investigation guide. Additional Legal-specific steps:
