# SEC-LEG-CO-001 — Legal: Email Forwarding / Data Repository Collection

| Field | Value |
|-------|-------|
| **Rule ID** | `SEC-LEG-CO-001` |
...
This rule applies the `GEN-CO-001` detection logic scoped to **Legal** infrastructure — restricting detection to hosts and systems specific to this sector using the watchlist and naming patterns below. The full KQL, investigation guide, and response actions from `GEN-CO-001` apply; this file adds the sector-specific host scope, context, and regulatory obligations.
...
```kql
// SEC-LEG-CO-001 | Sector scope applied to GEN-CO-001
...
- Rule name prefixed `SEC-LEG-CO-001`
...
Fee-earner workstations, DMS hosts, matter management servers are high-priority targets. Collection on these systems carries additional risk beyond the generic case:
...
Follow the `GEN-CO-001` investigation guide. Additional Legal-specific steps:
