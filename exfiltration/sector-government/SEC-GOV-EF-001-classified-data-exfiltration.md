# SEC-GOV-EF-001 — Government: Classified / Official Document Exfiltration

| Field | Value |
|-------|-------|
| **Rule ID** | `SEC-GOV-EF-001` |
| **MITRE Tactic** | Exfiltration |
| **Sector** | **Government** |
| **Severity** | See base rule `GEN-EF-001` — apply same severity |
| **Priority** | See base rule `GEN-EF-001` |
| **MS Tools** | Sentinel · Defender XDR · Defender for Endpoint |
| **Log Source** | See `GEN-EF-001` |
| **Sector Context** | Government workstations, policy systems, restricted infrastructure |
| **Regulatory** | GovAssure, NCSC CAF |

## Description

This rule applies the `GEN-EF-001` detection logic scoped to **Government** infrastructure — restricting detection to hosts and systems specific to this sector using the watchlist and naming patterns below. The full KQL, investigation guide, and response actions from `GEN-EF-001` apply; this file adds the sector-specific host scope, context, and regulatory obligations.

**Deploy this rule in addition to `GEN-EF-001` — not instead of it.**

## Sentinel KQL — Sector-Scoped Variant

```kql
// SEC-GOV-EF-001 | Sector scope applied to GEN-EF-001
// Full query: copy from GEN-EF-001 and add the WHERE clause below
...
Alternatively, create a copy of the `GEN-EF-001` Sentinel Analytics Rule with:
- Rule name prefixed `SEC-GOV-EF-001`
...
Government workstations, policy systems, restricted infrastructure are high-priority targets. Exfiltration on these systems carries additional risk beyond the generic case:
...
Follow the `GEN-EF-001` investigation guide. Additional Government-specific steps:
...
Follow `GEN-EF-001` response actions. Additional:
...
- Base rule: `GEN-EF-001` — full KQL and investigation guide
