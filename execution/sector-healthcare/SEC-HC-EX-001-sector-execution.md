# SEC-HC-EX-001 — Healthcare: PowerShell/WMI Execution on Sector Infrastructure

| Field | Value |
|-------|-------|
| **Rule ID** | `SEC-HC-EX-001` |
...
This rule applies the `GEN-EX-001` detection logic scoped to **Healthcare** infrastructure — restricting detection to hosts and systems specific to this sector using the watchlist and naming patterns below. The full KQL, investigation guide, and response actions from `GEN-EX-001` apply; this file adds the sector-specific host scope, context, and regulatory obligations.
...
```kql
// SEC-HC-EX-001 | Sector scope applied to GEN-EX-001
...
- Rule name prefixed `SEC-HC-EX-001`
...
Clinical workstations, EPR servers, PACS hosts are high-priority targets. Execution on these systems carries additional risk beyond the generic case:
...
Follow the `GEN-EX-001` investigation guide. Additional Healthcare-specific steps:
