# SEC-TECH-EX-001 — Technology: PowerShell/WMI Execution on Sector Infrastructure

| Field | Value |
|-------|-------|
| **Rule ID** | `SEC-TECH-EX-001` |
...
This rule applies the `GEN-EX-001` detection logic scoped to **Technology** infrastructure — restricting detection to hosts and systems specific to this sector using the watchlist and naming patterns below. The full KQL, investigation guide, and response actions from `GEN-EX-001` apply; this file adds the sector-specific host scope, context, and regulatory obligations.
...
```kql
// SEC-TECH-EX-001 | Sector scope applied to GEN-EX-001
...
- Rule name prefixed `SEC-TECH-EX-001`
...
Developer workstations, CI/CD servers, cloud management hosts are high-priority targets. Execution on these systems carries additional risk beyond the generic case:
...
Follow the `GEN-EX-001` investigation guide. Additional Technology-specific steps:
