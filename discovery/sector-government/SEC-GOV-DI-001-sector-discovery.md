# SEC-GOV-DI-001 — Government: Network Scanning on Sector Infrastructure

| Field | Value |
|-------|-------|
| **Rule ID** | `SEC-GOV-DI-001` |
...
This rule applies the `GEN-DI-001` detection logic scoped to **Government** infrastructure — restricting detection to hosts and systems specific to this sector using the watchlist and naming patterns below. The full KQL, investigation guide, and response actions from `GEN-DI-001` apply; this file adds the sector-specific host scope, context, and regulatory obligations.
...
```kql
// SEC-GOV-DI-001 | Sector scope applied to GEN-DI-001
...
- Rule name prefixed `SEC-GOV-DI-001`
...
Government workstations, policy systems, restricted infrastructure are high-priority targets. Discovery on these systems carries additional risk beyond the generic case:
...
Follow the `GEN-DI-001` investigation guide. Additional Government-specific steps:
