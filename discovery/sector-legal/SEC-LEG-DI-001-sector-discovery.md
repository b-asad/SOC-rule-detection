# SEC-LEG-DI-001 — Legal: Network Scanning on Sector Infrastructure

| Field | Value |
|-------|-------|
| **Rule ID** | `SEC-LEG-DI-001` |
...
This rule applies the `GEN-DI-001` detection logic scoped to **Legal** infrastructure — restricting detection to hosts and systems specific to this sector using the watchlist and naming patterns below. The full KQL, investigation guide, and response actions from `GEN-DI-001` apply; this file adds the sector-specific host scope, context, and regulatory obligations.
...
```kql
// SEC-LEG-DI-001 | Sector scope applied to GEN-DI-001
...
- Rule name prefixed `SEC-LEG-DI-001`
...
Fee-earner workstations, DMS hosts, matter management servers are high-priority targets. Discovery on these systems carries additional risk beyond the generic case:
...
Follow the `GEN-DI-001` investigation guide. Additional Legal-specific steps:
