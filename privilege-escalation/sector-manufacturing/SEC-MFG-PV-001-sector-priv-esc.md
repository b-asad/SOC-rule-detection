# SEC-MFG-PV-001 — Manufacturing: UAC Bypass on Sector Infrastructure

| Field | Value |
|-------|-------|
| **Rule ID** | `SEC-MFG-PV-001` |
...
This rule applies the `GEN-PV-001` detection logic scoped to **Manufacturing** infrastructure — restricting detection to hosts and systems specific to this sector using the watchlist and naming patterns below. The full KQL, investigation guide, and response actions from `GEN-PV-001` apply; this file adds the sector-specific host scope, context, and regulatory obligations.
...
```kql
// SEC-MFG-PV-001 | Sector scope applied to GEN-PV-001
...
- Rule name prefixed `SEC-MFG-PV-001`
...
Engineering workstations, OT jump servers, MES/ERP hosts are high-priority targets. Privilege Escalation on these systems carries additional risk beyond the generic case:
...
Follow the `GEN-PV-001` investigation guide. Additional Manufacturing-specific steps:
