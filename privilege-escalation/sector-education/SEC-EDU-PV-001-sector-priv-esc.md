# SEC-EDU-PV-001 — Education: UAC Bypass on Sector Infrastructure

| Field | Value |
|-------|-------|
| **Rule ID** | `SEC-EDU-PV-001` |
...
This rule applies the `GEN-PV-001` detection logic scoped to **Education** infrastructure — restricting detection to hosts and systems specific to this sector using the watchlist and naming patterns below. The full KQL, investigation guide, and response actions from `GEN-PV-001` apply; this file adds the sector-specific host scope, context, and regulatory obligations.
...
```kql
// SEC-EDU-PV-001 | Sector scope applied to GEN-PV-001
...
- Rule name prefixed `SEC-EDU-PV-001`
...
Student/staff workstations, research systems, VLE servers are high-priority targets. Privilege Escalation on these systems carries additional risk beyond the generic case:
...
Follow the `GEN-PV-001` investigation guide. Additional Education-specific steps:
