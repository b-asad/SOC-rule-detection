# SEC-EDU-LM-001 — Education: Pass-the-Hash / RDP Lateral Movement

| Field | Value |
|-------|-------|
| **Rule ID** | `SEC-EDU-LM-001` |
...
This rule applies the `GEN-LM-001` detection logic scoped to **Education** infrastructure — restricting detection to hosts and systems specific to this sector using the watchlist and naming patterns below. The full KQL, investigation guide, and response actions from `GEN-LM-001` apply; this file adds the sector-specific host scope, context, and regulatory obligations.
...
```kql
// SEC-EDU-LM-001 | Sector scope applied to GEN-LM-001
...
- Rule name prefixed `SEC-EDU-LM-001`
...
Student/staff workstations, research systems, VLE servers are high-priority targets. Lateral Movement on these systems carries additional risk beyond the generic case:
...
Follow the `GEN-LM-001` investigation guide. Additional Education-specific steps:
